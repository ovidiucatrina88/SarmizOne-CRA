  ...
  async calculateCostImpact(req: Request, res: Response) {
    try {
      ...
      for (const row of assignmentsResult.rows) {
        ...
        let cost = 0;
        const weight = parseFloat(assignment.weight) || 1.0;
        const costModuleId = assignment.cost_module_id as number;

        // Calculate cost based on cost type
        switch (assignment.cost_type) {
          case 'fixed':
            cost = parseFloat(assignment.cost_factor) * (parseFloat(assignment.quantity) || 1);
            break;
          case 'per_event':
            cost = parseFloat(assignment.cost_factor) * (parseFloat(assignment.quantity) || 1);
            break;
          case 'per_hour':
            cost = parseFloat(assignment.cost_factor) * (parseFloat(assignment.hours) || 1);
            break;
          case 'percentage':
        ...
        (weightedCosts as any)[costModuleId] = {
          id: costModuleId,
  ...
  async calculateAllRiskCosts(req: Request, res: Response) {
    try {
      ...
      for (const risk of risksResult.rows) {
        ...
        const riskMagnitude = parseFloat((risk as any).inherent_risk) || 0;
        let totalCost = 0;

        const riskCostModules: Array<{ moduleId: number; value: number; weight: number }> = [];
        ...
        if (assignmentsResult.rows.length > 0) {
          for (const row of assignmentsResult.rows) {
            ...
            riskCostModules.push({
              moduleId: assignment.cost_module_id,
              value: cost,
              weight: weight
            });
          }
