...
const generateSummaryData = (
  ...
  // Return stored exposure values if available, prioritizing primary fields
  if (currentExposure) {
    // Safely extract values and prioritize primary fields with fallbacks to legacy
    const minimumValue = currentExposure.minimumExposure || currentExposure.tenthPercentile || 0;
    const averageValue = currentExposure.averageExposure || currentExposure.mostLikely || 0;
    const maximumValue = currentExposure.maximumExposure || currentExposure.ninetiethPercentile || 0;
    
    // Only use pre-calculated data if we have valid values
    if (minimumValue > 0 && averageValue > 0 && maximumValue > 0) {
      console.log("Using pre-calculated risk summary data:", currentExposure);
      
      return {
        // Primary fields as source of truth
        minimum: minimumValue,
        average: averageValue,
        maximum: maximumValue,
        // Keep legacy fields for backward compatibility
        tenthPercentile: minimumValue,
        mostLikely: averageValue,
        ninetiethPercentile: maximumValue
      };
    }
  }
...
export function LossExceedanceCurve({ 
  ...
  const [thresholds, setThresholds] = useState({
    ...
    MEDIUM_ACCEPTANCE: DEFAULT_THRESHOLDS.MEDIUM_ACCEPTANCE,
    ...
    if (maxExposure > 0) {
      ...
      for (let i = 0; i <= numPoints; i++) {
        ...
        directCurveData.push({
          ...
          // Flag whether this is a threshold value point
      ...
      directCurveData.push({
        ...
        toleranceProbability: minExposure <= thresholds.FULL_ACCEPTANCE ? 100 : 
                             ...
                             minExposure <= thresholds.MEDIUM_ACCEPTANCE ? 50 :
      ...
      directCurveData.push({
        ...
        toleranceProbability: avgExposure <= thresholds.FULL_ACCEPTANCE ? 100 : 
                             ...
                             avgExposure <= thresholds.MEDIUM_ACCEPTANCE ? 50 :
      ...
      directCurveData.push({
        ...
        toleranceProbability: maxExposure <= thresholds.FULL_ACCEPTANCE ? 100 : 
                             ...
                             maxExposure <= thresholds.MEDIUM_ACCEPTANCE ? 50 :
      ...
      // Add explicit threshold points to the chart to ensure tolerance values are displayed correctly
      // 100% threshold (FULL_ACCEPTANCE)
      directCurveData.push({
        lossExposure: thresholds.FULL_ACCEPTANCE,
        // Calculate appropriate probability based on position between min and max exposure
        probability: 100 - (((thresholds.FULL_ACCEPTANCE - 0) / (maxExposure - 0)) * 100),
        previousProbability: null,
        toleranceProbability: 100,
        unacceptableRisk: 0,
        formattedLoss: formatExposure(thresholds.FULL_ACCEPTANCE),
        isThresholdPoint: true,
        thresholdType: "FULL_ACCEPTANCE",
      ...
      directCurveData.push({
        lossExposure: thresholds.ZERO_ACCEPTANCE,
        // Calculate appropriate probability based on position between min and max exposure
        probability: 100 - (((thresholds.ZERO_ACCEPTANCE - 0) / (maxExposure - 0)) * 100),
        previousProbability: null,
        toleranceProbability: 0,
        unacceptableRisk: 0,
        formattedLoss: formatExposure(thresholds.ZERO_ACCEPTANCE),
        isThresholdPoint: true,
        thresholdType: "ZERO_ACCEPTANCE",
        thresholdLabel: "0% acceptable"
      });
      ...
      <CardContent>
        ...
        <div className="mb-6 p-4 border border-border rounded-lg">
          ...
          <div className="grid grid-cols-1 gap-2">
            {/* Editable Threshold Cards - More compact and with 5 columns including 50% */}
            <div className="grid grid-cols-5 gap-1 mb-2">
              <div className="bg-primary/5 border border-primary/20 rounded-md p-2 shadow-sm">
                <p className="text-xs font-medium text-primary">100% acceptable</p>
                <div className="flex flex-col">
                  <Input 
                    ...
                    className="text-xs mt-1 text-right font-bold h-6" 
                  />
                  <span className="text-xs text-muted-foreground mt-1">{formatExposure(thresholds.FULL_ACCEPTANCE)}</span>
                </div>
              </div>
              
              <div className="bg-primary/5 border border-primary/20 rounded-md p-2 shadow-sm">
                <p className="text-xs font-medium text-primary">75% acceptable</p>
                <div className="flex flex-col">
                  <Input 
                    type="text" 
                    ...
                    className="text-xs mt-1 text-right font-bold h-6" 
                  />
                  <span className="text-xs text-muted-foreground mt-1">{formatExposure(thresholds.HIGH_ACCEPTANCE)}</span>
                </div>
              </div>
              
              <div className="bg-primary/5 border border-primary/20 rounded-md p-2 shadow-sm">
                <p className="text-xs font-medium text-primary">50% acceptable</p>
                <div className="flex flex-col">
                  <Input 
                    type="text" 
                    value={formatInputValue(thresholds.MEDIUM_ACCEPTANCE)} 
                    onChange={(e) => {
                      const rawValue = e.target.value.replace(/[^0-9]/g, '');
                      const value = Number(rawValue);
                      if (!isNaN(value)) {
                        updateThreshold('MEDIUM_ACCEPTANCE', value);
                      }
                    }} 
                    className="text-xs mt-1 text-right font-bold h-6" 
                  />
                  <span className="text-xs text-muted-foreground mt-1">{formatExposure(thresholds.MEDIUM_ACCEPTANCE)}</span>
                </div>
              </div>
              
              <div className="bg-primary/5 border border-primary/20 rounded-md p-2 shadow-sm">
                <p className="text-xs font-medium text-primary">25% acceptable</p>
                <div className="flex flex-col">
                  <Input 
                    type="text" 
                    ...
                    className="text-xs mt-1 text-right font-bold h-6" 
                  />
                  <span className="text-xs text-muted-foreground mt-1">{formatExposure(thresholds.LOW_ACCEPTANCE)}</span>
                </div>
              </div>
              
              <div className="bg-destructive/5 border border-destructive/20 rounded-md p-2 shadow-sm">
                <p className="text-xs font-medium text-destructive">0% acceptable</p>
                <div className="flex flex-col">
                  <Input 
                    type="text" 
                    ...
                    className="text-xs mt-1 text-right font-bold h-6" 
                  />
                  <span className="text-xs text-muted-foreground mt-1">{formatExposure(thresholds.ZERO_ACCEPTANCE)}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              <CartesianGrid 
                ...
                verticalPoints={[0, 50000000, 100000000, 150000000, 200000000, 250000000, 300000000, 350000000, 400000000, 450000000, 500000000, 550000000, 600000000]} // Match X-axis ticks
              />
              <XAxis 
                ...
                // Create evenly spaced ticks at $50M intervals up to $600M to visualize all risks
                ticks={[0, 50000000, 100000000, 150000000, 200000000, 250000000, 300000000, 350000000, 400000000, 450000000, 500000000, 550000000, 600000000]}
              />
              <YAxis 
                domain={[0, 100]}
              ...
              <Tooltip 
                content={({ active, payload }) => {
                  if (active && payload && payload.length) {
                    // Get the lossExposure and other data from the payload
                    const dataPoint = payload[0].payload;
                    const lossValue = dataPoint.lossExposure;
                    const isThresholdPoint = dataPoint.isThresholdPoint;
                    const thresholdType = dataPoint.thresholdType;
                    const thresholdLabel = dataPoint.thresholdLabel;

                    return (
                      <div className="bg-card p-2 border border-border shadow-md rounded-md">
                        {isThresholdPoint ? (
                          // Special formatting for threshold points
                          <>
                            <div className="flex items-center justify-between mb-1">
                              <p className="font-bold text-xs text-primary">{thresholdLabel}</p>
                              <p className="font-semibold text-xs text-card-foreground ml-2">{formatExposure(lossValue)}</p>
                            </div>
                            <p className="text-muted-foreground text-xs mb-1">
                              {thresholdType === "FULL_ACCEPTANCE" ? "100% acceptable risk" : 
                               thresholdType === "HIGH_ACCEPTANCE" ? "75% acceptable risk" : 
                               thresholdType === "LOW_ACCEPTANCE" ? "25% acceptable risk" : 
                               "0% acceptable risk"}
                            </p>
                          </>
                        ) : (
                          // Standard formatting for regular points
                        ...
                        {payload.map((entry, index) => {
                          ...
                          if (entry.dataKey === 'previousProbability' && entry.value !== null) {
                            // Calculate financial impact based on the probability
                            const value = entry.value || 0;
                            const probability = Math.round(value * 100) / 100;
                          ...
                          // For tolerance probability values
              ...
              {!useLinearCurve && showToleranceCurve && (
                <>
                  ...
                  <ReferenceArea
                    ...
                    x2={thresholds.MEDIUM_ACCEPTANCE}
                    y1={50}
                    y2={75}
                    fill="#DCF7EE"
                    fillOpacity={0.2}
                  ...
                  />
                  <ReferenceArea
                    x1={thresholds.MEDIUM_ACCEPTANCE}
                ...
                <>
                  ...
                  {!useLinearCurve && (
                    <>
                      ...
                      />
                      <ReferenceLine 
                        x={thresholds.MEDIUM_ACCEPTANCE} 
                        stroke="#10B981" 
                        strokeDasharray="3 3" 
                      />
                      <ReferenceLine 
